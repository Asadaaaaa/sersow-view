...
export default function Main({ category, params }) {
  ...
  useEffect(() => {
		async function detailsProject() {
			const res = await ManageProject(params.details[0], getCookie("auth"));
            ...
            categories: res.data.categories === null ? [] : res.data.categories,
            otherCtg: res.data.otherCtg,
            thumbnail: res.data.thumbnail ? res.data.thumbnail.url : null,
            ...
            program: res.data.program === null ? null : res.data.program.isUrl ? null : res.data.program.url,
            programUrl: res.data.program === null ? "" : !res.data.program.isUrl ? "" : res.data.program.url,
            paper: res.data.paper === null ? null : res.data.paper.isUrl ? null : res.data.paper.url,
            paperUrl: res.data.paper === null ? "" : !res.data.paper.isUrl ? "" : res.data.paper.url,
            code: res.data.code === null ? null : res.data.code.isUrl ? null : res.data.code.url,
            codeUrl: res.data.code === null ? "" : !res.data.code.isUrl ? "" : res.data.code.url,
            tags: res.data.tags === null ? [] : res.data.tags,
            contributors: res.data.contributors === null ? [] : res.data.contributors,
            published: res.data.published,
          });

          if (res.data.otherCtg) {
            setIsOther(true);
          }

          if (res.data.logo) {
            setLogo("data:image/jpeg;image/jpg;image/png;base64," + res.data.logo);
          }

          setDataStatus({
            ...dataStatus,
            thumbnail: res.data.thumbnail ? "success" : "ready",
            image1: res.data.image1 ? "success" : "ready",
            image2: res.data.image2 ? "success" : "ready",
            image3: res.data.image3 ? "success" : "ready",
            program: res.data.program ? (!res.data.program.isUrl ? "success" : "ready") : "ready",
            paper: res.data.paper ? (!res.data.paper.isUrl ? "success" : "ready") : "ready",
            code: res.data.code ? (!res.data.code.isUrl ? "success" : "ready") : "ready",
          });
  ...
  async function draftProject() {
    ...
    const dataSend = {
      ...
      contributors: data.contributors.length !== 0 ? data.contributors.map((item) => item.user_id) : null,
    }
                                    ...
                                    onClick={() => {
                                      const contributorExists = data.contributors.some((contributor) => contributor.id === item.id);
                                        if (!contributorExists) {
                                          setData((prevData) => ({
                                            ...prevData,
                                            contributors: [...prevData.contributors, { username: item.username, user_id: item.id }],
                                            tempContributors: "",
                                          }));
                                        }
                                    }}
